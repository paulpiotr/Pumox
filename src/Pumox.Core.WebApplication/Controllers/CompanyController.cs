...
namespace Pumox.Core.WebApplication.Controllers
{
    ...
    public class CompanyController : ControllerBase
    {
        ...
        public async Task<IActionResult> PutCompanyAsync(long id, Company company)
        {
            if (ModelState.IsValid)
            {
                if (null != _context && null != company)
                {
                    ...
                    {
                        return BadRequest(
                            $"Pole Id [{company?.Id}] w przekazanym obiekcie Company musi być oznaczone i jednoznaczne z polem Id [{id}] przekazanym parametrze URL!");
                    }
                    ...
                    if (null != employeeList)
                    {
                        foreach (Employee employee in employeeList)
                        {
                            ...
                            {
                                return BadRequest(
                                    $"Pole CompanyId [{employee?.CompanyId}] w przekazanej liście Employees musi być jednoznaczne z polem Id [{id}] przekazanym parametrze URL lub ustawione na NULL!");
                            }

                            if (employee.Id > 0 &&
                                await _context.Employee.Where(w => w.Id == employee.Id).Select(s => s.CompanyId)
                                    .FirstOrDefaultAsync() != company.Id)
                            {
                                return BadRequest(
                                    $"Pracownik [{employee?.Id}] w przekazanej liście Employees nie należy do firmy o Id [{id}] przekazanym parametrze URL!");
                            }
                        }
                    }
        ...
        public async Task<ActionResult<CompanySearchResultsViewModel>> PostSearchCompanyAsync(
            CompanySearch companySearch = null)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    // Keyword
                    var keyword = new SqlParameter("keyword", SqlDbType.NVarChar)
                    {
                        Value = $"%{companySearch?.Keyword}%"
                    };

                    // EmployeeDateOfBirthFrom
                    var employeeDateOfBirthFrom = new SqlParameter("employeeDateOfBirthFrom", SqlDbType.DateTime)
                    {
                        Value = companySearch?.EmployeeDateOfBirthFrom ?? DateTime.Now.AddYears(-120)
                    };

                    // EmployeeDateOfBirthTo
                    var employeeDateOfBirthTo = new SqlParameter("employeeDateOfBirthTo", SqlDbType.DateTime)
                    {
                        Value = companySearch?.EmployeeDateOfBirthTo ?? DateTime.Now.AddYears(-18)
                    };

                    // EmployeeJobTitles
                    List<sbyte> employeeJobTitlesList = companySearch?.EmployeeJobTitles?.Cast<sbyte>().ToList() ??
                                                        Enum.GetValues(typeof(Employee.JobTitles)).Cast<sbyte>()
                                                            .ToList();

                    // EmployeeJobTitles
                    var employeeJobTitles = new SqlParameter("employeeJobTitles", SqlDbType.VarChar)
                    {
                    ...
                    if (null != companySearch?.EmployeeDateOfBirthFrom &&
                        companySearch?.EmployeeDateOfBirthFrom != DateTime.MinValue &&
                        null != companySearch?.EmployeeDateOfBirthTo &&
                        companySearch?.EmployeeDateOfBirthTo != DateTime.MinValue)
                    {
                        ...
                        sqlStringBuilder.Append(
                            " (e.DateOfBirth BETWEEN @employeeDateOfBirthFrom AND @employeeDateOfBirthTo) ");
                    }
                    ...
                    sqlStringBuilder.Insert(0,
                        sqlStringBuilder.ToString().Length > 0
                            ? " SELECT e.CompanyId FROM [pcd].[Employee] e WHERE 1=1 AND "
                            : " SELECT e.CompanyId FROM [pcd].[Employee] e WHERE 1=1 ");
#if DEBUG
                    ...
                    List<long> employeeList = await _context.Employee
                        .FromSqlRaw(Regex.Replace(sqlStringBuilder.ToString(), @"\s{2,}", " ").Trim(), keyword,
                            employeeDateOfBirthFrom, employeeDateOfBirthTo, employeeJobTitles)
                    ...
                    return new CompanySearchResultsViewModel
                    {
                        Results = await _context.Company
                            .Where(w => null != employeeList && employeeList.Count > 0 &&
                                        employeeList.Contains(w.Id)).Include(i => i.Employees).ToListAsync()
                    };
                }
            }
            else
        ...
        private bool CompanyExists(long id) => _context.Company.Any(e => e.Id == id);
    }
}
